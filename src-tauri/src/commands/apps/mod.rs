...
pub fn load_apps_config(worktree_path: &str, project_path: &str) -> Vec<AppConfig> {
    ...
    // Fall back to `defaults` key in settings.json (lives in `extra`).
    let settings = crate::store::load();
    if let Some(defaults) = settings.extra.get("defaults") {
        if let Ok(config) = serde_json::from_value::<BandAppsConfig>(defaults.clone()) {
            if let Some(apps) = config.apps {
                return apps;
            }
        }
...
fn ensure_bundled_scripts() {
    let scripts_dir = crate::state::band_home().join("scripts");
